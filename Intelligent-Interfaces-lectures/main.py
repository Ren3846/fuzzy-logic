...
def create_UI():
    ...
    sg.theme('Dark Blue')
    layout = [[sg.Text('Якість обслуговування:')], [sg.Input('', enable_events=True, key='attendance', )],
              [sg.Text('Якість страв (у відсотках):')], [sg.Input('', enable_events=True, key='lectureNotes', )],
              [sg.Text('Чайові')], [sg.Input('', enable_events=True, key='result', )],
              [sg.Button('Submit', visible=True, bind_return_key=True)],
              [sg.Button('Exit')],
            ]

    window = sg.Window('Чайові', layout)
