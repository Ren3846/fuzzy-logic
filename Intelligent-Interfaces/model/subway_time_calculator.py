...
class SubwayTimeCalculator:
    __subway_station_coordinates = [
    ...
    @staticmethod
    def __latitude_to_km(latitude):
        return 110.574 * latitude

    @staticmethod
    def __longitude_to_km(longitude):
    ...
    def calculate(
        ...
        min_diff = None
        for subway_station in SubwayTimeCalculator.__subway_station_coordinates:
            latitude_diff = abs(subway_station[0] - latitude)
            longitude_diff = abs(subway_station[1] - longitude)

            latitude_diff_km = SubwayTimeCalculator.__latitude_to_km(
                latitude_diff
            )
            longitude_diff_km = SubwayTimeCalculator.__longitude_to_km(
                longitude_diff
            )

            general_diff = math.sqrt(
                latitude_diff_km ** 2 + longitude_diff_km ** 2
            )

            if min_diff is None or general_diff < min_diff:
                min_diff = general_diff

        return min_diff / SubwayTimeCalculator.__HUMAN_VELOCITY * 60
